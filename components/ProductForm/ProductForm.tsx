...
function ProductForm() {
    ...
    const { currentData } = useEditProductsDataContext()
    ...
    const handleInputChange = (id: string | string): void => {
        setCategoryId(id);
    }
    ...
    return (
        <form className={styles.form} onSubmit={handleSubmit}>
            <label htmlFor="name" className={styles.label}>
                Категория  товара
                <select value={categoryId} onChange={(e) => handleInputChange(e.target.value)} defaultValue={modalType == "product-edit" ? currentData?.id : undefined} id="name" className={styles.input} required>
            ...
            <label htmlFor="name" className={styles.label}>
                Описание
                <textarea value={description} onChange={(e) => setDescription(e.target.value)} id="name" minLength={3} className={styles.textarea} required></textarea>
            </label>
            {/* <label htmlFor="name" className={styles.label}>
        Фото 
